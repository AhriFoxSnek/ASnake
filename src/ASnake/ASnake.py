...
class Lexer(Lexer):
    ...
    META    = r'\$ *?((\w+(?![^+\-/*^~<>&\n()]*=)(?=[\n \]\)\$\[+:};(]))|(([^+\-/*^~<>&\n()[\],=]*|(\={2}))((=.*)|(?=\n)|$|(?=[,.]))))'
    FUNCMOD = r'@.*'
    PYDEF   = r'''(c|cp)?def +([\w.\[\d:,\] ]* )?(?:[a-zA-Z_][\w]*|[^\u0000-\u007F\s](?:[^\u0000-\u007F\s]|[\w])*) *\(([^()]|\((?:[^)]|'.*[()]+.*'|".*[()]+.*")*\))*\)*( *((-> *[\w\[\], {}.]+)? *):?)(?!return)'''
...
"""
                    ...
                    and ((lex[token].value in {'e','pi'} or lex[token].value.split('.')[-1] in {'e','pi'}) and any(m + '.' in trackingImported and (lex[token].value.split('.')[-1] in trackingImported[m + '.'] or lex[token].value in trackingImported[m + '.']) for m in ('math', 'numpy', 'scipy'))):
                        ...
                        elif metaCallSplit in metaDefExp and '=' in lex[token].value:
                            tmp = lex[token].value[lex[token].value.index('=') + 1:].strip().replace(' ','')
                            if tmp and '$' in tmp:
                                for key, val in inlineReplace.items():
                                    tmp = tmp.replace('$' + key, val)
                            expPrint.append(tmp.replace(' ',''))
                    ...
                    and ( (lex[token-1].type in {'STRING','NUMBER','BOOL'} and (lex[token+1].type in {'STRING','NUMBER','BOOL'} or isANegativeNumberTokens(token+1)) and (lex[token-2].type in typeConditionals+typeNewline+('AND','OR','LPAREN','ID','RETURN') )) \
                    or (isANegativeNumberTokens(token-2) and (lex[token+1].type in {'STRING','NUMBER'} or isANegativeNumberTokens(token+1)) and lex[token-3].type in typeConditionals+typeNewline+('AND','OR','LPAREN','ID','RETURN') ) ):
                            ...
                            and not (token+3 < len(lex) and lex[token+1].type == 'EXPONENT' and lex[token+3].type == 'EXPONENT') and not (token+4 < len(lex) and lex[token+1].type == 'EXPONENT' and isANegativeNumberTokens(token+2) and lex[token+4].type == 'EXPONENT'):
                                ...
                                if lex[tmppre].type in orderOfOps:
                                    if orderOfOps[lex[tmppre].type]==orderOfOps[lex[token+1].type]:
                                        if lex[tmppre].type == 'MINUS' and lex[token+1].type == 'PLUS':
                                            check = True
                                        elif lex[tmppre].type == 'DIVIDE' and lex[token+1].type == 'TIMES':
                                            check = True
                                        elif lex[tmppre].type == 'MODULO' and lex[token+1].type == 'DIVIDE':
                                            check = True
                                ...
                                for tmpi in range(token,len(lex)-1):
                                    if lex[tmpi].type in typeOperators+('NUMBER','LPAREN','RPAREN'):
                                        if   lex[tmpi].type == 'LPAREN': tmpscope-=1 ; tmpLastOperator=False
                                            ...
                                            if tmpLastOperator and lex[tmpi].type == 'MINUS' and lex[tmpi+1].type == 'NUMBER' and lex[tmpi-1].type in typeOperators:
